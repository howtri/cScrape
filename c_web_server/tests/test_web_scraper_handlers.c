...
static void test_handle_scrape_valid_url(void **state) {
    (void)state; // Unused
    ...
    expect_value(__wrap_send, len, strlen("SUCCESS: URL queued to be scraped.\n"));
    expect_value(__wrap_send, flags, 0);
    will_return(__wrap_send, strlen("SUCCESS: URL queued to be scraped.\n")); // Mock successful send

    assert_int_equal(handle_scrape_new_request(dummy_socket_fd, valid_url, p_queue), EXIT_SUCCESS);

    // Clean up
    queue_destroy(&p_queue);
}

static void test_handle_scrape_invalid_url(void **state) {
    (void)state; // Unused
    ...
    assert_int_equal(handle_scrape_new_request(dummy_socket_fd, invalid_url, p_queue), EXIT_FAILURE);

    // Clean up
    queue_destroy(&p_queue);
}

static void test_handle_invalid_request_success(void **state) {
    (void)state; // Unused

    // Expecting send to be called with the specific message
    expect_value(__wrap_send, sockfd, 42);
    expect_string(__wrap_send, buf, "SUCCESS: Invalid option, expected 1 or 2.\n");
    expect_value(__wrap_send, len, strlen("SUCCESS: Invalid option, expected 1 or 2.\n"));
    expect_value(__wrap_send, flags, 0);
    will_return(__wrap_send, strlen("SUCCESS: Invalid option, expected 1 or 2.\n"));
