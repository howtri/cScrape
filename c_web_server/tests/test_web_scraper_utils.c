...
static void check_filename_format(const char* url, const char* expected, size_t buffer_size) {
...
static void test_create_filename_valid_url(void **state) {
    (void)state; // Unused
    // Assuming util_djb_hash("http://example.com") returns a specific hash, e.g., 123456
    check_filename_format("http://example.com", "data/10040687989526346336.txt", 256);
}

// Test with NULL URL
static void test_create_filename_null_url(void **state) {
    (void)state; // Unused
    char buffer[256];
...
static void test_create_filename_empty_url(void **state) {
    (void)state; // Unused
    char buffer[256];
    int result = util_create_filename("", buffer, sizeof(buffer));
    // Expect success but with a specific hash for an empty string, if your hash function handles it
    assert_int_equal(result, EXIT_SUCCESS);
